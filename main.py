            ...
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                async with session.post(url, headers=headers, json=payload) as resp:
                    ...
                    if resp.status == 200:
                        ...
                        # Если content пустой, но есть reasoning — пытаемся извлечь ответ
                        if not content and reasoning:
                            print("EMPTY CONTENT, using reasoning_content")
                            # Ищем первую строку, начинающуюся с "**Имя**:" или просто берём первые 500 символов reasoning
                            ...
                            for line in lines:
                                if line.strip().startswith('**') and '**:' in line:
                                    ...
                                    break
                            if not content:
                                # Если не нашли формат, берём первые 500 символов reasoning
                                content = reasoning[:500]
                                print("FALLBACK: extracted from reasoning")
                        elif not content:
...
async def on_ready():
    ...
    print(
        f"✅ Акацуки бот запущен: {bot.user}"
    )
...
if __name__ == "__main__":
    ...
    asyncio.run(main())import os, re, json, random
from datetime import datetime
from zoneinfo import ZoneInfo
